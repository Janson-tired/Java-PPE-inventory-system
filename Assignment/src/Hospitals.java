 ...
 * @author jansu
 */
public class Hospitals implements Table_settings {

    // creates the gui
    JFrame frame = new JFrame("Hospitals");

    // creates the label
    JLabel hospital_id_label = new JLabel("Hospital id will be auto generated");
    JLabel hospital_name_label = new JLabel("Name: ");
    JLabel contact_label = new JLabel("Contact No.: ");
    JLabel address_label = new JLabel("Address: ");
    JLabel search_by_label = new JLabel("Search by: ");
    JLabel header_label = new JLabel("Hospitals");

    JPanel hospital_header = new JPanel();

    // create the text fields
    JTextField hospital_name_fields = new JTextField();
    ...
    JButton add_button = new JButton("Add hospital");
    ...
    JLabel hospital_image = new JLabel(hospital_icon);
    ImageIcon return_icon = new ImageIcon("return.png");

    // create the table
    DefaultTableModel model;
    JTable hospital_table;
    
    private String user_id;

    Hospitals(String user_id) throws IOException {
        ...
        String[] column = {"Hospital id", "Hospital name", "Contact No.", "Address"};
        model = new DefaultTableModel(column, 0);

        // add scrolling to the table
        hospital_table = new JTable(model);
        JScrollPane scrollPane = new JScrollPane(hospital_table);
        scrollPane.setBounds(500,175,600,150);

        hospital_header.setLayout(null);
        hospital_header.setBackground(new Color(0xc7a406));
        hospital_header.setBounds(0,0,1200,100);
        hospital_header.add(header_label);
        hospital_header.add(hospital_image);
        hospital_header.add(return_button);

        header_label.setFont(new Font("Monospaced", Font.BOLD, 50));
        header_label.setBounds(20,30,300,40);

        hospital_image.setBounds(300,35,30,30);

        // set the position for label
        hospital_id_label.setBounds(40,130,400,30);
        hospital_name_label.setBounds(40,175,150,30);
        ...
        hospital_id_label.setForeground(new Color(0xC3C3BD));
        hospital_id_label.setFont(new Font("Monospaced", Font.BOLD,15));
        hospital_name_label.setForeground(new Color(0xC3C3BD));
        hospital_name_label.setFont(new Font("Monospaced", Font.BOLD,15));
        ...
        hospital_name_fields.setBounds(200,175,250,25);
        ...
        frame.add(hospital_header);

        // add the labels
        frame.add(hospital_id_label);
        frame.add(hospital_name_label);
        frame.add(contact_label);
        frame.add(address_label);
        frame.add(search_by_label);

        // add the fields
        frame.add(hospital_name_fields);
        ...
        hospital_table.getSelectionModel().addListSelectionListener(event -> {
            if (!event.getValueIsAdjusting()) {

                // set the field content into the fields
                int selected_row = hospital_table.getSelectedRow();
                if (selected_row != -1) {
                    hospital_name_fields.setText((String) hospital_table.getValueAt(selected_row, 1));
                    contact_fields.setText((String) hospital_table.getValueAt(selected_row,2));
                    address_fields.setText((String) hospital_table.getValueAt(selected_row,3));
    ...
    public void add_record(ActionEvent e) {
        if (hospital_name_fields.getText().isEmpty() ||
                ...
                address_fields.getText().isEmpty())
        {
            ...
            try {
                ...
                // read the first number and add one to it
                ...
                new_id = Integer.parseInt(line_container[1]);
                new_id++;
            ...
            try {
                BufferedWriter writer = new BufferedWriter(new FileWriter("Hospitals.txt",true));
    ...
    public void update_record(ActionEvent e) {
        int selected_row = hospital_table.getSelectedRow();
        if (selected_row != -1) {
            hospital_table.setValueAt(hospital_name_fields.getText(), selected_row, 1);
            hospital_table.setValueAt(contact_fields.getText(), selected_row, 2);
            hospital_table.setValueAt(address_fields.getText(), selected_row, 3);
            JOptionPane.showMessageDialog(null, "record updated");

            try {
                BufferedReader reader = new BufferedReader(new FileReader("Hospitals.txt"));
                ...
                while ((line = reader.readLine()) != null) {
                    String[] line_container = line.split("<>");
                    if (Objects.equals(line_container[0], hospital_table.getValueAt(selected_row, 0))) {
                        temp_writer.write(
                                hospital_table.getValueAt(selected_row, 0) + "<>" +
                                        hospital_name_fields.getText() + "<>" +
                                        contact_fields.getText() + "<>" +
                                        address_fields.getText()
                        );
    ...
    public void delete_record(ActionEvent e) {
        ...
        if (selected_row != -1) {
            ...
            String delete_data = hospital_table.getValueAt(selected_row, 0).toString();
